"""Add empty canonical identity-link and migration-audit tables.
    ...
    op.execute(
        ...
        create policy "v1 memories own rows"
          ...
          using (
            ...
            and (conversation_id is null or exists (
              ...
              where c.id = v1_memories.conversation_id and c.user_id = auth.uid()
            ))
            and (project_id is null or exists (
              select 1 from public.v1_projects p
              where p.id = v1_memories.project_id and p.user_id = auth.uid()
            ))
          )
          with check (
            auth.uid() = user_id
        ...
        create policy "v1 conversations own rows"
          ...
          using (
            ...
            and (project_id is null or exists (
              select 1 from public.v1_projects p
              where p.id = v1_conversations.project_id and p.user_id = auth.uid()
            ))
          )
          with check (
            auth.uid() = user_id
        ...
        create policy "v1 messages own rows"
          ...
          using (
            ...
            and exists (
              select 1 from public.v1_conversations c
              where c.id = v1_messages.conversation_id and c.user_id = auth.uid()
            )
          )
          with check (
            auth.uid() = user_id
...
def downgrade() -> None:
    op.execute("drop table if exists public.v1_migration_audits")
    op.execute("drop table if exists public.v1_identity_links")
