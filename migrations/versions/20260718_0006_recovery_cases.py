...
Persists recovery case state and operation_key idempotency.
...
def upgrade() -> None:
    op.execute(
        ...
        -- At most one non-terminal case per subject / legacy identity.
...
def downgrade() -> None:
    op.execute(
        """
        drop table if exists public.v1_recovery_case_operations cascade;
        drop table if exists public.v1_recovery_cases cascade;
        """
    )
